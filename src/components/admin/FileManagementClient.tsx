...
import {
  ...
  deleteFile,
  uploadFile
} from '@/app/admin/dashboard/files/actions';
...
export default function FileManagementClient() {
  ...
  const fetchFiles = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        toast.error(
          `목록 조회 실패: ${result.error || '알 수 없는 오류가 발생했습니다.'}`
        );
      }
    } catch (error) {
  ...
  const handleFileUpload = async (
    ...
    if (file.type !== 'application/pdf') {
      toast.error(
        `잘못된 파일 형식: ${file.type}. PDF 파일만 업로드 가능합니다.`
      );
      return;
    }
    ...
    if (file.size > maxSize) {
      ...
      toast.error(
        `파일 크기 초과: ${fileSizeMB}MB. 최대 10MB까지 업로드 가능합니다.`
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(
          `업로드 실패: ${result.error || '알 수 없는 오류가 발생했습니다.'}`
        );
      }
    } catch (error) {
  ...
  const handleDeleteFile = async (fileId: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(
          `삭제 실패: ${result.error || '알 수 없는 오류가 발생했습니다.'}`
        );
      }
    } catch (error) {
        ...
        <div className="max-w-xl mx-auto space-y-4">
          ...
          <div className="space-y-3 p-3 border rounded">
            <div className="flex gap-2">
              ...
              <Button
                variant={
                  selectedFileType === 'portfolio' ? 'default' : 'outline'
                }
                size="sm"
